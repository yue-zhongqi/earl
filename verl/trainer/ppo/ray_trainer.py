...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_cpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            token_level_rewards_w_tools=data.batch["token_level_rewards_w_tools"],
            response_mask=data.batch["response_mask"],
            rewrite_mask=data.batch["rewrite_mask"],
            index=data.non_tensor_batch["uid"],
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE or self.config.algorithm.use_critic:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Log critic evaluation if enabled
                    critic_data_dir = self.config.trainer.get("critic_data_dir", None)
                    log_critic_freq = self.config.trainer.get("log_critic_freq", 100)
                    if critic_data_dir and self.global_steps % log_critic_freq == 0:
                        with marked_timer("dump_critic_evaluations", timing_raw, color="green"):
                            ...
                            data_dict = {
                                ...
                                "values": batch.batch["values"].cpu().float().numpy()
                            }
                            filename = os.path.join(critic_data_dir, f"{self.global_steps}.npz")
