...
@register_adv_est(AdvantageEstimator.CPO)  # or simply: @register_adv_est("cpo")
def compute_cpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    token_level_rewards_w_tools: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            # if norm_adv_by_std_in_grpo:
            #     # TODO: add -id2mean[index[i]] to the numerator to be better than group average
            #     scores[i] = (scores_w_tools[i] - scores_wo_tools[i]) / (id2std[index[i]] + epsilon)
            # else:
            #     scores[i] = scores_w_tools[i] - id2mean[index[i]]
            if rewrite_mask[i]:
                scores[i] = scores_w_tools[i] - scores_wo_tools[i]
            else:
                if norm_adv_by_std_in_grpo:
                    scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
                else:
