...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None, memory_limit_mb=None):
    ...
    elif data_source in ['basic_calculator_tutorial']:
        from . import basic_calculator_tutorial
        res = basic_calculator_tutorial.compute_score(solution_str, ground_truth, extra_info=extra_info)
    elif data_source in ['countdown3to4', 'countdown']:
        from . import countdown
        res = countdown.compute_score(solution_str, ground_truth, extra_info=extra_info)
    elif data_source in ['sorting']:
        from . import sorting
        res = sorting.compute_score(solution_str, ground_truth, extra_info=extra_info)
    elif data_source in ['calc_bench','big_math']:
        from . import calc_bench
        res = calc_bench.compute_score(solution_str, ground_truth, extra_info=extra_info)
    else:
